...
const combineOptionsData = (calls: OptionData[], puts: OptionData[]): CombinedOptionRow[] => {
  // Sort all strikes in descending order
...
const OptionsViewer = ({ className, apiKeys }: OptionsViewerProps) => {
  const [symbol, setSymbol] = useState<string>("");
  const [selectedSymbol, setSelectedSymbol] = useState<string>("");
  const { toast } = useToast();

  // Override API keys if provided
  const polygonKey = apiKeys?.polygon || POLYGON_API_KEY;
  const finnhubKey = apiKeys?.finnhub || FINNHUB_API_KEY;
  const alphaVantageKey = apiKeys?.alphaVantage || ALPHA_VANTAGE_API_KEY;

  const { data, isLoading, error } = useQuery({
    queryKey: ["options", selectedSymbol],
    queryFn: () => fetchOptionsData(selectedSymbol, toast),
  ...
  const handleSymbolSearch = (e: React.FormEvent) => {
    ...
    if (symbol.trim()) {
      ...
      toast({
        ...
        description: `Loading options data for ${symbol.toUpperCase().trim()}`,
      });
    }
  };

  const combinedData = data ? combineOptionsData(data.calls || [], data.puts || []) : [];
    ...
    <div className={`min-h-screen bg-gray-50 p-4 md:p-8 ${className || ''}`}>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {selectedSymbol && !isLoading && !error && (
          <Card className="p-6">
            <h2 className="text-xl font-semibold mb-4">Options Chain for {selectedSymbol}</h2>
